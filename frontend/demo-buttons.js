(function () {
  ...
  async function runFullDemo() {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:3001/run-demo");
  ...
  function build() {
    ...
    <div class="card" id="demoPanel" style="margin-bottom:20px;">
      ...
      <div style="display:flex;gap:8px;margin-top:12px;flex-wrap:wrap;">
        ...
        <button class="w-btn" style="background:#6c5ce7;color:white;font-size:11px;padding:8px;" onclick="MegaBytDemo.auto10()">
          🎲 COMPRA AUTOMÁTICA 10X
        </button>
  ...
  window.MegaBytDemo = {
    ...
    auto10: async () => {
      ...
      log("🚀 Automatic 10-ticket purchase started");

      const box = $("cmdBox");
      if (box) box.value = "Automatic 10-ticket Phantom flow running...";
      ...
      try {
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          log("Opening Phantom signature for ticket " + i + "/10...");
          buyBtn.click();
        ...
        log("Automatic 10-ticket purchase flow completed");
        if (box) box.value = "Automatic 10-ticket purchase flow completed";
      } catch (e) {
    ...
    runBackend: async () => {
      ...
      try {
        ...
        const r = await fetch("http://127.0.0.1:3001/run-demo");
        if (!r.ok) {
